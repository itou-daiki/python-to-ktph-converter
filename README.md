...
- 入力中のオートコンプリート
  ...
  - 上下キーで候補選択
  - Tab / Enter で決定
- 初回実行時に読み込む Pyodide による Python 実行
...
20件のサンプルを、授業で扱いやすい順にカテゴリ分けしています。選択すると難易度、概要、学習ポイント、入力例、確認課題が表示され、`読込`を押すとPython、共通テスト表記、フローチャートが同時に準備されます。

| カテゴリ | サンプル |
| --- | --- |
| 入出力と変数 | Hello World と入力、四則演算 |
| 条件分岐 | 得点の判定、FizzBuzz |
| 繰り返し | 合計とカウントダウン、逆順の繰り返し、九九の表 |
| 配列 | 配列の走査と平均、得点データの集計、条件に合う要素の抽出、回答の度数分布 |
| 探索と整列 | 線形探索、二分探索、バブルソート、選択ソート |
| 関数 | 関数の定義と戻り値、素数判定 |
| シミュレーション | さいころの度数実験 |
| 数のアルゴリズム | 最大公約数、10進数から2進数へ変換 |
...
- Python 実行: Pyodide 0.24.1（初回の`実行`時に遅延読込）
...
- Actionsでは公開に必要なHTML、`assets/`、`Sample/`だけを配信用ディレクトリへまとめます。
